...
fun minAndMax(number: Int): Int {
    val intToString = number.toString()
    val listOfInt = mutableListOf<Int>()
    if (number.toString().length > 1 && number.toString().length < 10 ) {
        for (char in intToString) {
            val charToInt = char.toString().toInt()
            listOfInt.add(charToInt)
        }
    }
    val sumValue = listOfInt.min() + listOfInt.max()
